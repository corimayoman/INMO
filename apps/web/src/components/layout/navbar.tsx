...
import { Globe, Heart, Bell, User, Menu, X, ChevronDown } from 'lucide-react';
    ...
    <header className="sticky top-0 z-50 bg-white border-b border-gray-100 shadow-sm">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <nav className="hidden lg:flex items-center gap-1">
            {navLinks.map((link) => (
              <Link key={link.href} href={link.href} className="px-3 py-2 text-sm font-medium text-gray-600 hover:text-brand-700 hover:bg-brand-50 rounded-lg transition-colors">
                {link.label}
              </Link>
            ))}
          </nav>
          ...
          <div className="flex items-center gap-2">
            {isAuthenticated ? (
              <>
                <Link href="/favorites" className="btn-ghost hidden sm:flex">
                  <Heart className="w-4 h-4" />
                </Link>
                <Link href="/dashboard/notifications" className="btn-ghost hidden sm:flex">
                ...
                <div className="relative group">
                  ...
                  <div className="absolute right-0 top-full mt-1 w-48 bg-white rounded-xl shadow-lg border border-gray-100 py-1 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all">
                    ...
                    <Link href="/dashboard/listings" className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">My Listings</Link>
              ...
              <>
                <Link href="/auth/login" className="btn-ghost hidden sm:flex text-sm">Sign in</Link>
      ...
      {mobileOpen && (
        <div className="lg:hidden border-t border-gray-100 bg-white px-4 py-3 space-y-1">
          {navLinks.map((link) => (
            <Link key={link.href} href={link.href} className="block px-3 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 rounded-lg" onClick={() => setMobileOpen(false)}>
              {link.label}
            </Link>
          ))}
        </div>
